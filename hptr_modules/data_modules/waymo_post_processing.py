...
class WaymoPostProcessing(nn.Module):
    ...
    def mpa_nms_multiagent(
        ...
        Args:
            valid: [n_scene, n_agent]
            trajs: [n_scene, n_agent, k_pred, n_step, 2], in local coordinate
            scores: [n_scene, n_agent, k_pred], normalized prob/ confidence
            type_thresh: in meters, list, len=3 [veh, ped, cyc]
            use_ade: if True, computes average distance between trajs, else distance between last positions
            agent_type: [n_scene, n_agent, 3], [veh, ped, cyc]
            normalize_across_agents: if True, normalizes scores across agents (n_agent dim),
                               if False, normalizes across predictions (k_pred dim)
