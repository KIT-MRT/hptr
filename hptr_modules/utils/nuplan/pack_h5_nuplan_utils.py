...
def mining_for_interesting_agents(
    ...
    """
    bicycles = FixedLengthDict(n_agent_prediction_challenge - 1)
    pedestrians = FixedLengthDict(n_agent_prediction_challenge - 1)
    vehicles = FixedLengthDict(n_agent_prediction_challenge - 1)
    ...
    for id, track in tracks.items():
        ...
        if type == 0:
            if id == "ego":
                ...
                vehicles.add(id, traj_score)
        # search for pedestrians
        elif type == 1:
            pedestrians.add(id, traj_score)
        # search for ciclists
        elif type == 2:
            bicycles.add(id, traj_score)

    # Create list with ids of agents for prediction challenge
    n_pedestrians_if_available = 2
    n_bicycles_if_available = 2
    track_ids_predict = ["ego"]
    track_ids_predict.extend(bicycles.get_top_x_keys(2))
    track_ids_predict.extend(pedestrians.get_top_x_keys(2))
    track_ids_predict.extend(
        vehicles.get_top_x_keys(
    ...
    if len(track_ids_predict) < n_agent_prediction_challenge:
        track_ids_predict.extend(
            pedestrians.get_top_x_keys(
                n_agent_prediction_challenge - len(track_ids_predict) + n_pedestrians_if_available
            )[n_pedestrians_if_available:]
        )
    if len(track_ids_predict) < n_agent_prediction_challenge:
        track_ids_predict.extend(
            bicycles.get_top_x_keys(
                n_agent_prediction_challenge - len(track_ids_predict) + n_bicycles_if_available
            )[n_bicycles_if_available:]
        )

    # TODO: Get the most interactive agent for the ego vehicle
    # Currently only the most interesting agent is selected
    track_ids_interact = ["ego"]
    track_ids_interact.extend(
        vehicles.get_top_x_keys(n_agent_interaction_challenge - 1)
    )
    # fill with pedestrians (first) and bicycles (second) if necessary
